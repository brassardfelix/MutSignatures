...
with open('StrandCoordinationLauch.sh', 'w') as file:
    for analysis in parameters:
        ...
        file.write(f'python3 RunSigProfilerAssignment.py -g {parameters[analysis]["genome"]} -i {parameters[analysis]["InputDir"]} -o {parameters[analysis]["OutputDir"]} -j {parameters[analysis]["jobname"]} -n {parameters[analysis]["nsims"]} -m {parameters[analysis]["matrix"]} -s "{samples}"\n')